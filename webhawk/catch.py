...
def get_data(log_file, log_content, log_type, log_size_limit, features, encoding_type):
    ...
    if log_type == 'os_processes':
        ...
        print("parsed process sample:", data[:3])
        data = pd.DataFrame.from_records(data)
        print("process columns:", data.columns)
        ...
        data = data.drop(['PGRP', 'PPID', 'UID'], axis=1, errors='ignore')
    else:
        try:
            if log_content==None:
...
def find_cves(findings):
    ...
    for finding in findings:
        if 'ai_advice' not in finding or not finding['ai_advice']:
            finding['ai_advice'] = finding.get('log_line', '')
    ...
    for finding in findings:
        if finding['severity'] == 'high':
            ...
            data = {
                "model": config['LLM']['model'],
                "prompt": (
                    'Extract all the CVE(s) from "{}". If you do not find anything, return "None" and do not return any other text. Return only a one line list of CVEs separated by commas.'.format(
                        finding.get('ai_advice', finding.get('log_line', ''))
                    )
                ),
                "stream": False,
            }
            ...
            try:
                ...
                if "CVE-" in resp_text:
                    finding['cve'] = resp_text.replace(" ", "").replace(",", " ")
                else:
                    finding['cve'] = ''
            except Exception as e:
                logging.info(f'Something went wrong getting CVE(s): {e}')
                finding['cve'] = ''
...
def get_llm_insights(findings):
    ...
    for finding in findings:
        ...
        try:
            logging.info(f'> Ollama URL: {url}')
            logging.info('> Before Ollama request')
            response = requests.post(url, json=data, timeout=120)
            logging.info('> After Ollama request')

            response.raise_for_status()
            finding['ai_advice'] = response.json().get('response', '').strip()
            logging.info(f'> AI advice length: {len(finding["ai_advice"])}')

        except Exception as e:
            logging.info(f'> Ollama request failed: {e}')
            finding['ai_advice'] = ''
...
def main():
    ...
    if args['show_plots']:
        ...
        if args['log_type'] != 'os_processes':
            ...
            plot_data([data['%CPU'], data['%MEM']], 'Plotting %CPU and %MEM')
            if 'RES' in data.columns:
                plot_data([data['%CPU'], data['%MEM'], data['RES']], 'Plotting %CPU, %MEM and RES')
