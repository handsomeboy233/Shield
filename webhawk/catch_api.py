...
def main(hostname,log_file,logs_content):
    ...
    for finding in all_findings:
        ...
        logging.info(f'> Start processing finding: line={finding.get("log_line")}')

        if ai_advice == True:
            logging.info('> Getting LLM advices..')
            logging.info('> Before get_llm_insights')
            updated_finding = get_llm_insights([finding])
            logging.info('> After get_llm_insights')

            raw_ai_advice = updated_finding[0]['ai_advice']
            logging.info(f'> raw_ai_advice: {raw_ai_advice}')

            try:
                logging.info('> Before json.loads')
                clean_ai_advice = raw_ai_advice.replace("```json", "").replace("```", "").strip()
                json_ai_advice = json.loads(clean_ai_advice)
            ...
            if data_is_ok == True:
                logging.info('> Before mapping fields')
